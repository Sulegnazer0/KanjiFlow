...
assert.equal(dictionary.length, 683);
assert.equal(new Set(dictionary.map(itemId)).size, dictionary.length);
assert.equal(dictionary.filter(item => item.tipo === "kanji").length, 470);
assert.equal(Object.keys(KANJI_EXAMPLES).length, 470);
...
assert.equal(stats.total, 683);
assert.equal(stats.newCount, 683);
...
console.log("✓ Datos únicos y 470 ejemplos de kanji");
