...
assert.equal(dictionary.length, 353);
assert.equal(new Set(dictionary.map(itemId)).size, dictionary.length);
assert.equal(dictionary.filter(item => item.tipo === "kanji").length, 140);
assert.equal(Object.keys(KANJI_EXAMPLES).length, 140);
...
for (const code of localeCodes) {
    ...
    for (const lessonId of ["recommended", "hira-basic-1", "kata-basic-1", "kana-special", "kanji-4", "kanji-n4-1", "kanji-n4-2", "kanji-n4-3", "all"]) {
...
assert.equal(stats.total, 353);
assert.equal(stats.newCount, 353);
...
console.log("✓ Datos únicos y 140 ejemplos de kanji");
