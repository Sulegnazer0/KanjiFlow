...
assert.equal(dictionary.length, 333);
assert.equal(new Set(dictionary.map(itemId)).size, dictionary.length);
assert.equal(dictionary.filter(item => item.tipo === "kanji").length, 120);
assert.equal(Object.keys(KANJI_EXAMPLES).length, 120);
...
for (const code of localeCodes) {
    ...
    for (const lessonId of ["recommended", "hira-basic-1", "kata-basic-1", "kana-special", "kanji-4", "kanji-n4-1", "kanji-n4-2", "all"]) {
...
assert.equal(stats.total, 333);
assert.equal(stats.newCount, 333);
...
console.log("✓ Datos únicos y 120 ejemplos de kanji");
